...
/// THE RELATIVE DIFFERENCE BELOW WHICH TWO SCORES ARE THE SAME MEASUREMENT
/// TAKEN ON DIFFERENT MACHINES — the audit's alarm threshold, and nothing else
/// reads it. Reuse stays EXACT: any difference at all means the stored number
/// is not the one this run would compute, and that test must not soften.
///
/// A score is deterministic within a platform and only within one. Measured on
/// one board row, same code and same data: the Linux runner writes
/// `1.1070976928071057` where another host computes `1.1070976928071055` — two
/// ULP, 2e-16 relative, from the host's own libm rather than from anything this
/// repository decides. So an alarm on exact inequality fires on every row the
/// day a runner image moves, which is the shape of alarm nobody reads twice.
/// Four orders above that noise and many below any change a build can make.
const PLATFORM_NOISE: f64 = 1e-12;
...
fn main() {
  ...
  mode={} mods=[{}] evolutions=[{}] arcanes=[{}] valence={}",
    ...
    if verify {
        ...
        // HOW FAR IT MOVED, AND NOT ONLY THAT IT DID. Exactness is the right
        // test for REUSE — any difference and the stored number is not the one
        // this run would compute — but it is the wrong alarm, because a score is
        // reproducible bit for bit only within a PLATFORM: the same code and
        // data on another host lands a couple of ULP away, so a runner image
        // bump would report every row moved and teach a reader to ignore the
        // one that matters. `beyond` is the count that survives that, and the
        // exact count is still published beside it.
        ...
        let beyond = moved.iter().filter(|&&(_, w, n)| rel(w, n) > PLATFORM_NOISE).count();
        ...
        eprintln!(
            "verify-result: compared={compared} moved={} beyond={beyond} stale={stale} worst={worst:e}",
            moved.len()
        );
        if compared < floor {
