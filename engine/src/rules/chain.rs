...
impl Layout {
    ...
    pub fn build(bodies: &[Vec2], splash: Splash, spec: Spec) -> Self {
        let caught = (0..bodies.len())
            .filter(|&i| crate::rules::space::caught_by_blast(bodies[i].distance(splash.at), splash.radius_m))
            .map(|i| i as u32)
            .collect();
        let near = bodies
            .iter()
...
pub fn acquired(
    ...
    let muzzle = crate::rules::space::muzzle(player_at, aim_at);
    let mut v: Vec<(f64, u32)> = bodies
        .iter()
        .enumerate()
        .filter_map(|(i, b)| {
            if !crate::rules::space::within(b.distance(player_at), range_m) {
                return None;
            }
            let off = crate::rules::space::off_axis_deg(muzzle, aim_at, *b);
            crate::rules::space::within(off, cone_deg).then_some((off, i as u32))
        })
        .collect();
    v.sort_by(|x, y| {
        x.0.partial_cmp(&y.0).unwrap_or(std::cmp::Ordering::Equal).then(x.1.cmp(&y.1))
...
pub fn resolve_with(
    ...
    // them (`rules::space::struck_along`). It was ONE body until 2026-08-17, which was
    ...
    seeds.extend((0..bodies.len()).filter(|&i| {
        ...
        // in one place (`rules::space::caught_by_blast`,). So the
        // radius a splash really seeds over is its own plus a body radius.
        !struck.contains(&i)
            && crate::rules::space::caught_by_blast(bodies[i].distance(splash.at), splash.radius_m)
    }));
