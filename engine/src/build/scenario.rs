...
//! [`BUILD_AXES`](crate::board::builds::BUILD_AXES)'s sibling, for the other half of
...
impl Capability {
    ...
    pub fn of(self, spec: &crate::data::weapons::WeaponSpec) -> bool {
...
mod tests {
    ...
    fn sp(id: &str) -> &'static crate::data::weapons::WeaponSpec {
        crate::data::weapons::spec(id).expect("roster weapon")
    }
    ...
    fn the_first_missing_capability_decides() {
        let artax = crate::data::weapons::spec("artax").unwrap();
        ...
        let both = crate::data::weapons::roster()
    ...
    fn every_capability_is_exercised_and_explains_itself() {
        ...
        for c in caps {
            ...
            let missing: Vec<_> = crate::data::weapons::roster().filter(|s| !c.of(s)).collect();
