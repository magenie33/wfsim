...
"#,
    ...
    fn a_demolishers_three_restrictions_are_three_different_mechanics() {
        ...
        let immune: Vec<crate::rules::damage::DamageType> = d
            .status_immunities
            .iter()
            .filter_map(|s| crate::rules::damage::DamageType::from_name(s))
            .collect();
        let share = |ty: crate::rules::damage::DamageType| {
            let v = crate::rules::damage::DamageVector::new()
                .with(ty, 75.0)
                .with(crate::rules::damage::DamageType::Slash, 25.0);
            let mut rng = crate::rules::rng::Rng::new(0xD3E0);
            (0..4000)
                .filter(|_| crate::rules::status::draw_proc_type(&v, &immune, &mut rng) == Some(ty))
                .count()
        };
        // IMPACT IS NULLIFIED AND STILL ROLLS — 3/4 of this vector, which it
        // would lose entirely if it were read as an immunity.
        let impacts = share(crate::rules::damage::DamageType::Impact);
        assert!(
        ...
        assert_eq!(share(crate::rules::damage::DamageType::Radiation), 0, "Radiation cannot land");
        let vr = crate::rules::damage::DamageVector::new()
            .with(crate::rules::damage::DamageType::Radiation, 75.0)
            .with(crate::rules::damage::DamageType::Slash, 25.0);
        let mut rng = crate::rules::rng::Rng::new(0xD3E0);
        let slashes = (0..4000)
            .filter(|_| {
                crate::rules::status::draw_proc_type(&vr, &immune, &mut rng)
                    == Some(crate::rules::damage::DamageType::Slash)
            })
            .count();
...
"#;
        ...
        let t = s
            ...
            .expect("builds");
        assert_eq!(t.status_immunities, vec![crate::rules::damage::DamageType::Slash]);
        // …the Slash it cannot BLEED from still lands its damage in full, and
        // the Heat it takes none of is still drawn for procs.
        assert_eq!(t.type_mods.faction.get(crate::rules::damage::DamageType::Slash), 1.0);
        assert_eq!(t.type_mods.faction.get(crate::rules::damage::DamageType::Heat), 0.0);
    }
...
"#;
    ...
    fn the_override_redirects_the_column_without_touching_the_faction() {
        ...
        assert_eq!(t.type_mods.faction.get(crate::rules::damage::DamageType::Void), 1.5);
        ...
        let gt = g
            ...
            .unwrap();
        assert_eq!(
            gt.type_mods.faction.get(crate::rules::damage::DamageType::Puncture),
            1.5
        );
        assert_eq!(
            gt.type_mods.faction.get(crate::rules::damage::DamageType::Radiation),
            0.5
        );
    }
