...
mod tests {
  ...
  ")
    ...
    fn known_mod(id: &str) -> Option<crate::model::ModDef> {
        crate::data::mods::classes()
            .into_iter()
            .find_map(|c| crate::data::mods::class_pool(c).into_iter().find(|m| m.id == id))
    }

    fn known_arcane(id: &str) -> Option<&'static crate::data::arcanes::ArcaneDef> {
        crate::data::arcanes::slots()
            .into_iter()
            .find_map(|s| crate::data::arcanes::for_slot(s, id))
    }
    ...
    fn overlay_keys_reference_real_ids() {
        ...
        let damage_types: Vec<&str> = crate::rules::damage::DamageType::ALL
            .iter()
            .map(|t| t.name())
            .collect();
        for (code, spec) in locales() {
            for id in spec.weapons.keys() {
                assert!(
                    crate::data::weapons::spec(id.as_str()).is_some(),
                    "i18n/{code}: unknown weapon id '{id}'"
                );
            }
            for id in spec.enemies.keys() {
                assert!(
                    crate::data::enemies::all().iter().any(|e| &e.id == id),
            ...
            for id in spec.auras.keys() {
                assert!(
                    crate::data::auras::by_id(id.as_str()).is_some(),
            ...
            for id in spec.shards.keys() {
                ...
                let d = crate::data::shards::all().iter().find(|x| x.id == shard);
            ...
            for id in spec.warframe_mods.keys() {
                assert!(
                    crate::data::warframes::mod_by_id(id).is_some(),
            ...
            for (id, ranks) in &spec.warframe_mod_descriptions {
                let m = crate::data::warframes::mod_by_id(id)
            ...
            for (id, ranks) in &spec.warframe_arcane_descriptions {
                let a = crate::data::warframes::arcane_by_id(id)
            ...
            for id in spec.evolutions.keys().chain(spec.evolution_descriptions.keys()) {
                assert!(
                    crate::data::evolutions::get(id.as_str()).is_some(),
