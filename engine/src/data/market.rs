...
fn base_entry(id: &str) -> Option<&'static crate::data::weapons::WeaponSpec> {
    let by_id = |want: &str| crate::data::weapons::all().iter().find(|w| w.id == want);
    let w = by_id(id)?;
...
mod tests {
    ...
    fn every_key_is_an_id_this_app_still_has() {
        let mods: Vec<String> = crate::data::mods::classes()
            .iter()
            .flat_map(|c| crate::data::mods::class_pool(c))
            .map(|m| m.id.to_string())
            .collect();
        for id in market().mods.keys() {
            assert!(mods.contains(id), "market.yaml names a mod we do not have: {id}");
        }
        let weapons: Vec<&str> = crate::data::weapons::all().iter().map(|w| w.id.as_str()).collect();
        ...
        let arcanes: Vec<&str> = crate::data::arcanes::slots()
            .iter()
            .flat_map(|s| crate::data::arcanes::slot_pool(s))
            .map(|a| a.id.as_str())
            .collect();
        for id in market().arcanes.keys() {
            assert!(arcanes.contains(&id.as_str()), "market.yaml names an arcane we do not have: {id}");
        }
        let families: Vec<&str> = crate::data::weapons::all()
            .iter()
        ...
        let stats: Vec<String> = riven_classes()
            .iter()
            .flat_map(|c| crate::build::rivens::pool(c))
            .map(|s| s.id.clone())
            .collect();
