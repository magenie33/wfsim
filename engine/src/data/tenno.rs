...
pub struct Tenno {
    ...
    #[serde(default)]
    pub auras: Vec<crate::data::auras::AuraPick>,
...
impl Tenno {
    ...
    pub fn squad(&self, class: &str) -> SquadEffects {
        ...
        use crate::data::shards::ShardEffect;
        ...
        let strength = 1.0
            + self
                .auras
                .iter()
                .filter_map(|p| crate::data::auras::by_id(&p.id))
        ...
        for pick in &self.auras {
            let Some(d) = crate::data::auras::by_id(&pick.id) else { continue };
        ...
        for pick in &self.shards {
            match crate::data::shards::resolve(pick) {
    ...
    ///
    /// [`ShardEffect::unmodelled_reason`]: crate::data::shards::ShardEffect::unmodelled_reason
    pub fn shard_bonuses(&self, slot: &str) -> StatBonuses {
        ...
        for pick in &self.shards {
            ...
            let Some(d) = crate::data::shards::all().iter().find(|x| x.id == pick.shard) else {
                continue;
            };
...
/// `aiming` as a bool threaded through `build::loadout::resolve`, beside a separate
...
mod tests {
    ...
    fn a_shard_pays_its_own_slot_and_the_rest_say_why_they_do_not() {
        ...
        let mut t = crate::data::tenno::default_tenno().clone();
        assert_eq!(t.shard_bonuses("primary"), crate::data::tenno::StatBonuses::default());
        ...
        // never report.
        for d in crate::data::shards::all() {
            for o in &d.options {
                let paid = !matches!(
                    o.at(false),
                    crate::data::shards::ShardEffect::StatusChance(..)
                        | crate::data::shards::ShardEffect::CritChance(..)
                        | crate::data::shards::ShardEffect::Armor(_)
                        | crate::data::shards::ShardEffect::EnergyMax(_)
                        | crate::data::shards::ShardEffect::StatusStackCap(..)
                );
                assert_eq!(
                    paid,
