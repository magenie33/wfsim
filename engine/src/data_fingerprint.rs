...
mod tests {
    ...
    /// A FILE THIS PIPELINE WRITES CANNOT BE PART OF WHAT IT HASHES.
    ///
    /// `data/` is hashed whole into every row, and `board_state.yaml` is
    /// written into it by the scorer and committed by the publish step — so a
    /// publish that listed one new build moved the global hash and made EVERY
    /// row stale, buying a full rescore with the board's own growth. The
    /// workflow states this rule for `boards/`, which it keeps outside the
    /// hash for exactly this reason; it holds for anything generated under
    /// `data/` too, and this asserts the input set rather than the list.
    #[test]
    fn a_file_this_pipeline_writes_is_not_hashed_into_every_row() {
        let hashed: Vec<&str> = crate::data::files_under("")
            .map(|(p, _)| p)
            .filter(|p| !ignored(p) && !PER_ENTITY.contains(&family_of(p)))
            .collect();
        assert!(!hashed.is_empty(), "nothing is in the global bucket at all");
        for p in &hashed {
            assert!(
                !p.ends_with("board_state.yaml"),
                "{p} is generated by the scorer and hashed into every row's fingerprint"
            );
        }
    }
