...
//!
//! `body` answers who took a damage instance and `Attacker` answers who dealt
...
fn every_instance_is_credited_to_an_attacker() {
    let r = one_fight();
    let dealt: f64 = r.dealt.by_attacker().0.iter().sum();
...
fn a_solo_fight_credits_the_wielder_and_nobody_else() {
    let r = one_fight();
    let by = r.dealt.by_attacker().0;
    assert!(by[Attacker::WIELDER.0] > 0.0, "the wielder dealt nothing");
    assert!(
