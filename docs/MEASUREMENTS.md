...
### The one thing that does NOT reconcile, and is left open

Every DoT tick in both blocks is **×1.0286 (= 36/35)** above what the engine
computes, consistently, across three independent brackets:
...
The direct hits pin the base at exactly 35 (98, 105, 137, 168 all land on the
digit), so the DoT half behaves as though the base were 36. `DOT_COEFFICIENT`
is 0.5 and every one of these is `0.5 × 36 × bracket`.

It is NOT changed here. A rounding rule, a per-weapon quirk and a wrong
coefficient all fit these nine rows, and the coefficient reaches every elemental
DoT in the app. The reading that would separate them is a weapon whose base
damage is ODD and does not halve cleanly, with a single plain Toxin DoT and
nothing else on the build: base 45 gives 22.5 under the current model against
23.1 under a 36/35 one, which no rounding can blur. Requested, not yet taken.
