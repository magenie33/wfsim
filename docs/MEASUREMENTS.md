...
* **THE TORID'S BASE FORM, which is the same shot on the other side.** It is
  `Multiplying` where the Incarnon form is `Adding`, so ONE reading answers both
  open questions at once — the CO class and the base. Read the NON-crit number
  (its crit chance is 15%), with Plentiful Mayhem for a panel of 131 and a
  base-damage mod on, at 1 stack against 1 status type:

  | hypothesis | reads |
  |---|---|
  | Multiplying, evolution fed | **486** |
  | Multiplying, not fed | **453** |
  | Adding, fed | **400** |
  | Adding, not fed | **387** |

  Four outcomes, well separated. The bare non-crit should read `131 x 2.65 =
  347` first, to confirm nothing else is on the build.
* **A Multiplying entry, any Multiplying entry.** The 24 pairs left at the old
  default rest on nothing measured. `cargo run --release --bin co_candidates`
  ranks them.
