...
| **The 1-damage floor, off armour and below zero** | The engine ASSUMES every hit that lands deals at least 1, on every pool and with no armour, and that a hit driven below zero (a Damage total under −100%, e.g. −145% on a Verglas Prime riven) deals exactly 1 — the owner's reading (docs/MECHANICS.md §8). wiki `Armor` states the floor for armour alone, per DAMAGE TYPE ("a Braton will always do a minimum of 3"), and says "Non-Armor sources of damage reduction do not have such a behavior". Three readings settle it: a weapon with a sub-1 physical component (a fraction of Impact) on an unarmoured Simulacrum dummy — 0 or 1 on that type's pop-up; the same weapon on an armoured one — one floor per type or one per hit; and any build whose Damage bonus sums below −100% — 0, 1, or a heal. |
