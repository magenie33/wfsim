...
| **Coda Sporothrix** (`coda_sporothrix`, and `sporothrix` behind it) | **The barb's 0.9 s eruption**, which both yamls admit they do not model — and the delay is an infobox field, not prose. The panel is not the question: every stat checks out against the rendered page, explosion included (48 = 25 Slash + 23 Viral, 2.0 m, 5% / 3.0×, 55%). Five readings, in this order. **Does the barb still erupt when the direct hit KILLS?** — that is what decides whether the delay is scheduling or lost damage, and with 11 rounds at 1.83/s there are two barbs in the air at once. **Does the eruption crit?** — the infobox gives it 5% / 3.0×, so a yellow number is a one-shot confirmation. **Does it take the Coda's valence element**, which is stated as raising "the listed base damage", and **does it take base-damage mods** — the page declines to say for either. **Two status rolls**: *"Initial hit and explosion apply status separately"*, and the eruption carries innate Viral. |
| **The 1-damage floor, off armour and below zero** | The engine floors a hit on HEALTH at 1 only after armour reduced it (`fight::target_state`, `.max(1.0)`), per instance; wiki `Armor` states it per DAMAGE TYPE ("a Braton will always do a minimum of 3") and says "Non-Armor sources of damage reduction do not have such a behavior". Believed otherwise and unread: that the floor holds with NO armour too, and that a Damage bonus below −100% (a riven malus, e.g. −145% on a Verglas Prime) deals 1 rather than 0. Three readings settle all of it: a weapon with a sub-1 physical component (a fraction of Impact) on an unarmoured Simulacrum dummy — 0 or 1 on that type's pop-up; the same weapon on an armoured one — one floor per type or one per hit; and any build whose Damage bonus sums below −100% — 0, 1, or a heal. Until then that Verglas row is banked at 0. |
| **Catabolyst, Coda Catabolyst** (`catabolyst`, `coda_catabolyst`, `critical_mutation`) | The reload grenade (MECHANICS §7.2) is modelled off the wiki and nothing read in game. **When the throw lands** — the engine lands it halfway through the reload, a placeholder (the throw costs no time either way); time reload-press to damage number against a dummy at 5 m and 15 m, which also reads the travel time. **The Coda's fan** — read "45 degree angle towards the reticle" as 45° edge to edge: stand 8 m off a lone dummy and count blast numbers per throw (three if the outer two reach it, one if not). **Critical Mutation's reach** — the engine pays it on the grenade only (U35's wording); with a full pile, does the BEAM's yellow-number rate move? **Its decrement** — one step per THROW or per GRENADE on the Coda: throw at a lone dummy with a known pile and read the grenade's crit numbers before and after. |
| **The frame's timeline** (`data::casting`) | Four unmeasured times and three beliefs. Time, input to input: Transference out and back with nothing between (`TRANSFERENCE_SECONDS_UNMEASURED`, 2 s), the Chained Sling added to it (1 s), the school's ability added to it (1 s), and Hysteria's cast to the first claw swing (`CAST_SECONDS_UNMEASURED`, 1 s). The beliefs: the claws SNAPSHOT — summon Hysteria inside Sling Strength's 20 s, read a normal attack, let the sling lapse and read it again, equal if the snapshot holds and 1/1.4 if the claws read strength live; Power Ramp's repeated cast gains nothing and re-arms one stack; Arcane Fury and Strike refresh one buff rather than stacking a second. |
| **Demolisher Devourer** (`demolisher_devourer`) | **Where M89's 0.8 actually sits.** Both readings behind it are HEADSHOT bleeds, and `3.0 × 0.8² = 1.92` — so "a 0.8 in the faction bracket" and "this unit's head multiplier is 1.92 rather than the page's 3.0" predict the same two numbers, and the Bane does not tell them apart because it moves only the faction term. Two shots do, with an unmodded Aklex Prime: the **body, non-critical bleed** (33.9 if the 0.8 is real, 52.9 if the head is what is wrong) and the **head, non-critical DIRECT** hit (360 against 288). The engine carries the first answer today. |
