...
| **The frame's timeline** (`data::casting`) | Three unmeasured times and one belief. Time, input to input, on top of M105's 1 s Transference: the Chained Sling (`CHAINED_SLING_SECONDS_UNMEASURED`, 1 s), the school's ability (1 s), and Hysteria's cast to the first claw swing (`CAST_SECONDS_UNMEASURED`, 1 s). The belief: Arcane Fury and Strike refresh one buff rather than stacking a second. |
