...
| **The frame's timeline** (`data::casting`) | Three unmeasured times and one belief. Time Transference out → Chained Sling → Transference back, input to input (`OPERATOR_SLING_SECONDS_UNMEASURED`, 3 s), and Hysteria's cast, input to first claw swing (`CAST_SECONDS_UNMEASURED`, 1 s). The belief is that the claws SNAPSHOT: summon Hysteria inside Sling Strength's 20 s, read a normal attack's number, let the sling lapse and read it again — equal if the snapshot holds, 1/1.4 of it if the claws read strength live. |
