...
## MEASUREMENT > WIKI > DE's PUBLIC EXPORT

ONE ORDER, FOR EVERY FIELD, whenever two sources disagree: an **in-game
measurement** (`docs/MEASUREMENTS.md`), then **the wiki** — its pages over its
data modules (§"INSIDE THE WIKI"), and the CN wiki for Chinese names — then
**DE's Public Export**.

**The export fills what the wiki does not state.** `scripts/de_export.py`
downloads it (English and Chinese) into `vendor/public-export/`: `uniqueName`
(every data file's `internal_name`), `baseDrain` / `fusionLimit`, polarity,
rarity, compatibility tags, the texture a card is drawn with, and DE's own card
text in every language. Where the wiki states the same field and disagrees, the
wiki wins — and inside the wiki a page's rank table wins over
`Module:Mods/data`, which is wrong about `BaseDrain` / `MaxRank` for about
twenty mods.

It is never the source of a mechanical number. `levelStats` is the card as DE
...
## `Weapon Comparison/CSV` — every gun in one fetch
...
## The wiki's own METHOD page, and where the mechanics pages are
...
document was only about the second.
...
| drop tables / World State API / EE.log | first-party data (the Public Export itself is a source — §"MEASUREMENT > WIKI > DE's PUBLIC EXPORT") |
...
### THE PER-WEAPON CATALOGS ARE CACHED LOCALLY
...
## "RANGE" ON A GUN MEANS TWO DIFFERENT THINGS
...
The order is §"MEASUREMENT > WIKI > DE's PUBLIC EXPORT"; nothing can show which
source is wrong about a field while only one is consulted, so each is
cross-checked, and the table says where each field is read.

| field | read from | on a conflict |
| --- | --- | --- |
| `base_drain`, `max_rank` | the wiki PAGE's rank table | `Module:Mods/data` is wrong for ~20 mods; the page and the export agreed 8 of 8 (Point Strike, Split Chamber, Metal Auger, Barrel Diffusion, Convulsion, Deep Freeze, Gunslinger, Suppress) |
| `internal_name` | DE's Public Export `uniqueName` | the wiki's `InternalName` wins if it disagrees, and the split is a finding |
| `polarity`, `rarity`, `exilus` | wiki module | `crosscheck.py` reports a split with the export |
| per-rank effect VALUES, dispositions | the wiki page | the export's `levelStats` is rounded display (Argon Scope 2 s for 1.5 s), and its dispositions can disagree with the wiki's — the wiki wins until measured |
| card text, English and localized | DE's Public Export | the wiki's module can lag a patch (Update 44.0's weak-point cards); the page's own prose saying so is the wiki winning over its module |
| everything mechanical | wiki | a disagreement is itself the finding — `crosscheck.py` reports SOURCE-SPLIT |
...
and Combustion Beam are both tagged `PRIMARY` and neither can go on the Torid
 — they need a CONTINUOUS weapon, which DE's own internal
...
- `--type Shotgun` reports 10 MISSING: **Bounty Hunter, Crash Shot, Flak Shot,
  ...
  Hands) DO ship and are in the engine allowlist.
...
### The WEAPON-EXCLUSIVE sweep, and the two ways a mod is refused
...
### The POOL-TAG sweep, and the pool that was a name and nothing else
...
## The faction vulnerability column, checked cell by cell
