...
const PROJECT_FACTS = {"rust_tests": 972, "browser_checks": 64, "commits": 2245, "first_commit_day": "2026-07-23"};
...
const lanes = async () => {
  ...
  for (let i = 1; i < n; i++) laneAt(i);
  return pool.slice(0, n).map((l, i) => (l && !l.dead ? l : laneAt(i)));
};
...
function ensureGains(axis, repaint) {
  ...
  // one that did.
  if (gainScan.running && gainScan.want === gainKey()
  ...
  let last = 0;
  // A REJECTION HERE STOPPED THE CALCULATOR FOR GOOD. This was launched without
  // `await` and without a catch, so anything that threw inside left `running`
  // true and the counter frozen — and every later request returned early
  // against a scan that had already died.
  scanGains(axis, (st) => {
    const now = Date.now();
    if (!st.running || now - last > 250) {
      last = now; renderQuickCalc(); renderCalcStatus(); repaint();
    }
...
function renderScenarioFields(ids, opts = {}) {
  ...
  if (ids.limits) {
    $(ids.limits).innerHTML = ammoField(w, sim) + buffTriggersField();
  }
