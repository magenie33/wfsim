...
const BUILD_ID = "ff20fe34+ · 2026-08-31 06:19Z";
...
const PROJECT_FACTS = {"rust_tests": 920, "browser_checks": 58, "commits": 2128, "first_commit_day": "2026-07-23"};
...
let sim = { enemy: "thrax_centurion", level: 9999, steel_path: true, eximus: null, headshot_pct: 100, aiming: true,
  ...
  solo_weapon: false,
  // WHAT TRIGGERS NO BUFF HERE, as event ids — the events still happen and
  // still score. Empty is the engagement this app has always run.
  // See `buffEventsField` (Limits).
  buff_events_off: [],
  frame: "",
...
/// FAMILY's members, while `mergeRivenFamilyLists` passes the ONE weapon whose
/// list it is moving — a family-wide sweep there would rewrite the OTHER
/// variant's builds, which point at their own card of the same name.
///
...
// (`SHARED_DOMAINS`) and there is no "⇤ import".
//
...
// where edits on the Laetum show up on the Dual Toxocyst. Copying ACROSS
// weapons is the deliberate "⇤ import" on each bar. No count cap: presets live
// in the reader's localStorage rather than with us.
...
function renderScenarioFields(ids, opts = {}) {
  ...
  if (ids.limits) {
    $(ids.limits).innerHTML = ammoField(w, sim) + buffEventsField();
  }
  ...
  if (opts.readonly) {
    boxes.forEach((box) => box.querySelectorAll("[data-k],[data-xk],[data-bev]").forEach((el) => {
      el.disabled = true;
  ...
  boxes.forEach((box) =>
    box.querySelectorAll("[data-bev]").forEach((el) =>
      el.addEventListener("change", () => { toggleBuffEvent(el.dataset.bev, el.checked); })));
  boxes.forEach((box) =>
...
function buildShortfalls() {
  ...
  // much Forma you own. A panel counting only how many slots are full lets two
  // such builds onto the board, refused on every scoring run since.
  //
  ...
  // itself — so a page judging the live layout would refuse exactly the people
  // who had not polarized yet. The floor is every slot matched, the best any
  // layout can reach and what `mods::fit` concludes.
  //
...
/// WHAT TRIGGERS NO BUFF IN THIS FIGHT, one switch per kind — the subject is
/// docs/BUFFS.md and the vocabulary is `/api/meta.buff_events`.
///
/// IT SWITCHES OFF A TRIGGER, NOT AN EVENT. The run still kills, still hits
/// weak points, still procs, and the score still counts every one of them;
/// what stops is the BUFF those would have granted. The label said "this fight
/// hands out no kills", which reads as a fight where nothing dies — a claim
/// about the arena, and the wrong one.
///
...
/// assume. Every kind is drawn on every weapon, for the reason a scenario
/// describes a FIGHT and not a build — a term that appeared and vanished with
/// the mods equipped would be a ruler that changed shape between weapons.
...
function buffEventsField() {
  const ids = (META.buff_events || []).map((e) => e.id);
  if (!ids.length) return "";
  const off = new Set(sim.buff_events_off || []);
  return `<div class="bev-h">${escHtml(tr("no buff is triggered by"))}</div>
    <div class="bev-row">${
    ids.map((id) => `<label class="bev${off.has(id) ? " on" : ""}"><input type="checkbox" data-bev="${
...
/// two spellings of one fight compare as two fights.
function toggleBuffEvent(id, on) {
  const set = new Set(sim.buff_events_off || []);
  if (on) set.add(id); else set.delete(id);
  const known = (META.buff_events || []).map((e) => e.id);
  sim.buff_events_off = known.filter((x) => set.has(x))
    .concat([...set].filter((x) => !known.includes(x)));
  markScenarioDirty();
  renderSim();
}

/// Is this card denied by the fight? The RUN answers it from the same table
