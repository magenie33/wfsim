...
const BUILD_ID = "d1715787+ · 2026-08-31 05:17Z";
...
const PROJECT_FACTS = {"rust_tests": 920, "browser_checks": 58, "commits": 2126, "first_commit_day": "2026-07-23"};
...
let sim = { enemy: "thrax_centurion", level: 9999, steel_path: true, eximus: null, headshot_pct: 100, aiming: true,
  ...
  solo_weapon: false,
  // WHAT THIS FIGHT NEVER HANDS OUT, as event ids — empty is the engagement
  // this app has always run. See `buffEventsField` (Limits).
  buff_events_off: [],
  frame: "",
...
function renderBuffCards(box, list, cfg, have, opts = {}) {
  ...
  const card = (b) => {
    ...
    // "this fight will not pay it" are the one pair a reader must tell apart.
...
/// WHAT THIS FIGHT NEVER HANDS OUT, one switch per kind — the subject is
...
function buffEventsField() {
  ...
  return `<div class="bev-h">${escHtml(tr("this fight hands out no"))}</div>
    <div class="bev-row" title="${escHtml(
      tr("nothing that has to be earned this way is earned at all — every buff below that asks for it is off for the whole run"))}">${
