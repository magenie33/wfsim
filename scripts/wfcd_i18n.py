...
Usage:
  python scripts/wfcd_i18n.py check [--i18n PATH] [--locale zh]
      Diff data/i18n/<locale>.yaml against WFCD: report entries that
      disagree, and ids WFCD could name that we haven't filled.
  python scripts/wfcd_i18n.py fill --section mods --section arcanes
      [--i18n PATH] [--locale zh]
      Rewrite the given empty-or-existing sections of the overlay from
...
def main() -> int:
    ...
    ap.add_argument("mode", choices=["check", "fill"])
    ...
    overlay_path = ROOT / "data" / "i18n" / f"{args.locale}.yaml"
