...
| `companions/` | the hosts a robotic weapon can be held by (loaded by `engine::data::companions`; the stat block is `tenno/sentinel.yaml`'s) | `id`, `name` |
| `companion_mods/` | the robotic companion pool, every card `unmodelled` — nothing reads a wielder's stat and no precept is run | `rarity`, `polarity`, `base_drain`, `max_rank`, `compat` (DE's `compatName`), `precept`, `description`, `effects` |
