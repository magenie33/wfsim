...
| `tenno/` | the **player** (loaded by `engine::tenno_data`), shaped like a Warframe — the field names are the wiki module's. The stat block is still INERT (nothing reads it yet; `health`/`shield` are PLACEHOLDERS at 1, not Warframe stats), but `state` is LIVE: a mod's `condition: while_invisible` / `while_airborne` is asked of it, so a while-Invisible mod pays exactly when the Tenno is invisible | `health`, `shield`, `overguard`, `armor`, `energy`, `sprint`, `state.{invisible,airborne}` |
